...
struct ContentView: View {
    ...
    @State private var currentFilter: CIFilter = CIFilter.sepiaTone()   // so we`ve got sepiaTone() by default
...
//        image = Image(uiImage: inputImage)
    ...
    func applyProcessing() {
        ...
        if inputKeys.contains(kCIInputRadiusKey) {
            currentFilter.setValue(filterIntensity * 200, forKey: kCIInputRadiusKey)   // for Radius we time by 200 the value to make radius bigger
        }
